<script>
	...
	import { submitOrder, approveCurrency, getBalanceOf, getProduct } from '../lib/methods'
	...
	import { address, productId, product, currencyLabel, leverage, size, margin, marginPlusFee, isSubmittingShort, isSubmittingLong, prices, allowances, transactionCount } from '../lib/stores'
	...
	$: getBalance($currencyLabel, $leverage, $address, $transactionCount);
...
<div class='order' class:loading={$isSubmittingLong || $isSubmittingShort}>
	...
	<div class="coll">
		Collatoral Asset : <div class='collbtn' on:click={() => {showModal('Currencies')}} data-intercept="true">{formatCurrency($currencyLabel)}</div>			
	</div>
	<div class="coll">
		Leverage (in multiple) : <div class='collbtn' on:click={() => {showModal('Leverage')}} data-intercept="true">{formatToDisplay($leverage)}x</div>			
	</div>
	...
	<div class='buttons'>
		...
		{:else}
		<Button isRed={true} isLoading={$isSubmittingShort} label='Short/Sell' onClick={() => {_submitNewPosition(false)}} /> <Button isLoading={$isSubmittingLong} label='Long/Buy' onClick={() => {_submitNewPosition(true)}} />
		{/if}
	</div>
	...
	{#if !$address}
	<div class='note'><a data-intercept="true" on:click={() => {showModal('Connect')}}>Connect your wallet</a> on Aurora to get started.</div>
	{:else if $address && balance * 1 == 0}
	<div class='note'>Get ETH to Aurora to start trading.</div>
	{/if}
