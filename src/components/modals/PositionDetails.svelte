<script>
	...
	async function setRows(_prices) {
		...
		let execPrice = formatToDisplay(data.price);
		let curPrice = formatToDisplay(_prices[data.productId]);
		let liqPrice = formatToDisplay(liquidationPrice);

		if(productName == "BTC-USD") {
			productName = "Yuga-Index";
			execPrice = formatToDisplay(data.price*5.2);
			curPrice = formatToDisplay(_prices[data.productId] * 5.2);
			liqPrice = formatToDisplay(5.2*liquidationPrice);
		}
		...
		rows = [
			...
			},
			{
				...
				value: data.price * 1 > 0 ? execPrice : 'Settling'
			},
			{
				label: 'Current Price',
				value: curPrice
