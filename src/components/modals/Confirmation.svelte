<script>
	...
	async function calculateRows(price) {
		...
		let feeAmount = (fee / 100) * data.size;
		...
		let entryPrice = formatToDisplay(averagePrice);

		if(productName == "BTC-USD") {
			entryPrice = formatToDisplay(averagePrice*5.2);
			productName = "Yuga-Index";
			liqPrice = liqPrice*5.2;
		}
...
<Modal title={existingPosition ? 'Resulting Position' : 'New Position'} showHeader={true} showCancel={true}>
	...
	<Button isYellow={true} wrap={true} isLoading={isSubmitting} onClick={_submitNewPosition} label='Confirm Order' />
</Modal>