...
export function setTitle(product, price) {
  if (get(currentPage) == "trade") {
    document.title = `${product} ${price} || Ariel Finance`;
  }
}
...
export function formatToDisplay(amount, maxPrecision, fixPrecision) {
  ...
  if (
    !fixPrecision &&
    amount * 1 <= 100 &&
    (amount * 1).toFixed(6) * 1 == Math.round(amount * 1)
  )
...
export function setCachedLeverage(_productId, _leverage) {
  ...
  if (cl) {
    ...
    localStorage.setItem(
      "leverage",
      JSON.stringify({ [_productId]: _leverage })
    );
  }
}
...
export function loadRoute(path, isInitial) {
  if (
    ...
    document.title = `Market | Ariel Finance`;
  } else if (path.includes("/liquidity")) {
    component.set(Pool);
    currentPage.set("pool");
    document.title = `Liquidity | Ariel Finance`;
  }
  hydrateData();
}
...
export function formatOrders(orders, info) {
  ...
  for (const o of orders) {
    ...
    formattedOrders.push({
      ...
      leverage: o.margin.toNumber()
        ? (formatUnits(o.size) * 1) / (formatUnits(o.margin) * 1)
        : 0,
    });
    i++;
  }
...
export function formatPositions(positions, info) {
  ...
  for (const p of positions) {
    ...
    formattedPositions.push({
      ...
      currencyLabel: getCurrencyLabelFromAddress(
        p.currency || (info[i] && info[i].currency)
      ),
...
export async function getUPL(position, latestPrice) {
  ...
  if (latestPrice) {
    ...
    if (position.isLong) {
      ...
      upl =
        (position.size * (latestPrice * 1 - position.price * 1)) /
        position.price;
    } else {
      latestPrice = latestPrice * (1 - priceImpact / 100);
      upl =
        (position.size * (position.price * 1 - latestPrice * 1)) /
        position.price;
    }
    // Add interest
...
export async function getInterest(position) {
  ...
  if (
    !position.price ||
    !position.timestamp ||
    now < position.timestamp * 1 + 900
  ) {
    interest = 0;
  } else {
    interest =
      (position.size *
        ((productInfo.interest * 1 || 0) / 100) *
        (now - position.timestamp * 1)) /
      (360 * 24 * 3600);
  }
  ...
  return (
    ...
    (baseSpread * 100 +
      maxSlippage *
        (1 -
          Math.exp(
            -1 * Math.pow(size / maxLiquidity[_currencyLabel], slippageExponent)
          )))
  );
}
