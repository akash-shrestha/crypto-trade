...
export async function getProduct(productId) {
  ...
  productCache[productId] = formatProduct(
    productId,
    await contract.getProduct(toBytes32(productId))
  );
...
export async function getAllowance(currencyLabel, spenderName) {
  ...
  const allowance = formatUnits(
    await contract.allowance(address, spenderContract.address),
    18
  );
...
export async function getPoolInfo(currencyLabel, reloading) {
  ...
  try {
    ...
    const withdrawFee =
      dataCache[currencyLabel].withdrawFee ||
      formatUnits(await contract.withdrawFee(), 2);
    dataCache[currencyLabel].withdrawFee = withdrawFee;

    const utilizationMultiplier =
      dataCache[currencyLabel].utilizationMultiplier ||
      formatUnits(await contract.utilizationMultiplier(), 2);
    dataCache[currencyLabel].utilizationMultiplier = utilizationMultiplier;

    const utilization =
      poolBalance * 1
        ? (openInterest * utilizationMultiplier) / poolBalance
        : 0;
...
export async function getOldPoolInfo(currencyLabel) {
  ...
  try {
    ...
    const claimableReward = await getClaimableReward(
      currencyLabel,
      false,
      true
    );
...
export async function withdraw(currencyLabel, amount, isOld) {
  ...
  try {
    ...
    monitorTx(tx.hash, isOld ? "pool-withdraw-old" : "pool-withdraw", {
      currencyLabel,
    });
    hideModal();
...
export async function collectPoolReward(currencyLabel, isOld) {
  const contract = await getContract(
    isOld ? "oldpoolrewards" : "poolrewards",
    true,
    currencyLabel
  );
  if (!contract) throw "No contract available.";
  ...
  try {
    ...
    monitorTx(tx.hash, isOld ? "pool-collect-old" : "pool-collect", {
      currencyLabel,
    });
...
export async function getCapPoolInfo() {
  ...
  for (const currencyLabel in currencies) {
    claimableRewards[currencyLabel] = await getClaimableReward(
      currencyLabel,
      true
    );
