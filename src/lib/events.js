...
export async function initEventListeners() {
  ...
  tradingContract.on(
    tradingContract.filters.NewOrder(null, _address),
    handleEvent
  );
  tradingContract.on(
    tradingContract.filters.PositionUpdated(null, _address),
    handleEvent
  );
  tradingContract.on(
    tradingContract.filters.ClosePosition(null, _address),
    handleEvent
  );
...
async function handleEvent() {
  ...
  if (ev.event == "PositionUpdated") {
    ...
    showToast(
      `Order executed: ${size} ${currency} ${
        args.isLong ? "long" : "short"
      } on ${fromBytes32(args.productId)} at ${price}.`,
      "success"
    );
  }
  ...
  if (ev.event == "ClosePosition") {
    ...
    showToast(
      `Position closed on ${fromBytes32(
        args.productId
      )}. P/L: ${pnl} ${currency}.`,
      "success"
    );
