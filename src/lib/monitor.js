...
import { provider, transactionCount } from "./stores";
...
async function handleTxComplete(type, details) {
  transactionCount.update(transactionCount => transactionCount+1)
  if (type == "submit-new-position") {
    ...
    showToast(
      `Deposited into ${formatCurrency(details.currencyLabel)} pool.`,
      "success"
    );
    await getPoolInfo(details.currencyLabel);
  } else if (type == "pool-withdraw") {
    showToast(
      `Withdrew from ${formatCurrency(details.currencyLabel)} pool.`,
      "success"
    );
    await getPoolInfo(details.currencyLabel);
    ...
    showToast(
      `Collected rewards from ${formatCurrency(details.currencyLabel)} pool.`,
      "success"
    );
    await getPoolInfo(details.currencyLabel);
