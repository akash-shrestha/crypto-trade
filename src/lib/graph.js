...
const graph_url = "https://api.thegraph.com/subgraphs/name/geekybot/arielsub";
...
export async function getUserOrders() {
  ...
  let graph_keys = graph_orders.map((e) => {
    return e.id;
  });

  let keys = _events.map((e) => {
    return e.args.key;
  });
...
export async function getUserPositions() {
  ...
  for (const p of _positions) {
    if (
      _raw_positions[i] &&
      _raw_positions[i].size &&
      _raw_positions[i].size.toString() * 1 > 0
    ) {
