...
export const productId = writable(
  localStorage.getItem("productId") || "ETH-USD"
);
export const product = writable({});

export const currencyLabel = writable(
  localStorage.getItem("currencyLabel") || "weth"
);
...
export const marginPlusFee = derived(
  [size, leverage, product],
  ([$size, $leverage, $product]) => {
...
export const slippage = derived(
  [size, productId, currencyLabel],
  ([$size, $productId, $currencyLabel]) => {
    // console.log('p', $size, $productId, $prices, $currencyLabel);

    if (!$size || !$productId || !$prices[$productId] || !$currencyLabel)
      return 0;
      ...
      (baseSpread * 100 +
        maxSlippage *
          (1 -
            Math.exp(
              -1 *
                Math.pow($size / maxLiquidity[$currencyLabel], slippageExponent)
            )))
    );
  },
  0
...
export const enhancedPositions = derived(
  [orders, positions],
  ([$orders, $positions]) => {
...
// Chart
export const chartResolution = writable(
  localStorage.getItem("chartResolution") || 900
);
export const chartLoading = writable();

// Wallet
export const provider = writable(null);
export const chainId = writable(1313161555);
